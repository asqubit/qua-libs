...
if simulate:
    ...
    while results.is_processing():
        ...
        for i in range(num_qubits):
            ...
            plt.cla()
            plt.plot((rr.LO_frequency + rr.intermediate_frequency) / u.MHz + dfs / u.MHz, np.abs(s_data[-1]), ".")
            plt.title(f"{rr.name}")
            plt.ylabel(r"R=$\sqrt{I^2 + Q^2}$ [V]")
            plt.sca(axss[1, i])
            plt.cla()
            plt.plot((rr.LO_frequency + rr.intermediate_frequency) / u.MHz + dfs / u.MHz, signal.detrend(np.unwrap(np.angle(s_data[-1]))), ".")
    ...
    for i, (ax, rr, s) in enumerate(zip(axs, resonators, s_data)):
        try:
            ...
            data[f"{rr.name}"] = {"resonator_frequency": int(rr.RF_frequency), "successful_fit": True}
