...
if simulate:
    ...
    while results.is_processing():
        ...
        ax1 = plt.subplot(211)
        plt.cla()
        plt.plot(((rr.LO_frequency + rr.intermediate_frequency) + frequencies) / u.MHz, R, ".")
        plt.ylabel(r"$R=\sqrt{I^2 + Q^2}$ [V]")
        plt.subplot(212, sharex=ax1)
        plt.cla()
        plt.plot(((rr.LO_frequency + rr.intermediate_frequency) + frequencies) / u.MHz, signal.detrend(np.unwrap(phase)), ".")
