...
if simulate:
    ...
    while results.is_processing():
        ...
        for i, q in enumerate(qubits):
            ...
            plt.cla()
            plt.plot((q.xy.LO_frequency + q.xy.intermediate_frequency + dfs) / u.MHz, np.abs(s))
            plt.grid(True)
            plt.ylabel(r"R=$\sqrt{I^2 + Q^2}$ [V]")
            plt.title(f"{q.name} (f_01: {q.xy.rf_frequency / u.MHz} MHz)")
            plt.subplot(2, num_qubits, num_qubits + i + 1)
            plt.cla()
            plt.plot((q.xy.LO_frequency + q.xy.intermediate_frequency + dfs)/ u.MHz, np.unwrap(np.angle(s)))
            plt.grid(True)
    ...
    for i, q in enumerate(qubits):
        try:
            ...
            res = fit.reflection_resonator_spectroscopy(
                (q.xy.LO_frequency + q.xy.intermediate_frequency + dfs) / u.MHz, -np.unwrap(np.angle(s_data[i])), plot=True
            )
    ...
    # }
    # Save data from the node
    node_save(machine, "qubit_spectroscopy", data)
